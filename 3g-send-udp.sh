...
if [ "$#" -ne 3 ]; then
	echo "./3g-send-udp.sh <ip addres> <port> <message>"
	exit 0
fi
...
echo -n "Opening UDP port... "
MODEM_OUTPUT=`echo "AT+CIPOPEN=0,\"UDP\",,,9000" | ./atinout - $MODEM_DEVICE -`
...
echo -n "Sending UDP packet '${3}'... "
P=$3
L=${#P}
T="AT+CIPSEND=0,${L},\"${1}\",${2}"
